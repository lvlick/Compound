...
defmodule Compound.TCP.Server do
  ...
  def init({serverId, port}) do
    ...
    case :gen_tcp.listen(port, [:binary, packet: :line]) do
      {:ok, socket} -> {:ok, %{listen_socket: socket, config: config, con_sup: nil, loop: nil, counter: 0, clients: [], default_callback: nil}, {:continue, :init}}
  ...
  def handle_continue(:init, state) do
    ...
    con_sup_specs = DynamicSupervisor.child_spec([strategy: :one_for_one, name: "[" <> state.config.id <> "]TCP.Connection.Supervisor" |> String.to_atom()]) |> Map.put(:id, "[" <> state.config.id <> "]TCP.Connection.Supervisor")
    {:ok, con_sup} = Supervisor.start_child(
      Compound.TCP.Supervisor, con_sup_specs
    )
